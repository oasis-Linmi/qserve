...
# os.environ["NUM_GPU_PAGE_BLOCKS"] = "6400"

# 遍历每个模型路径
for model_path in model_paths:
    print(f"Testing model: {model_path}")
    
    # 生成合法的输出文件名：将路径中的 "/" 替换为 "_"，并去除前导的 "_"（如果有）
    output_filename = model_path.replace("/", "_").lstrip("_") + "-output_with_max_bs_test_only_decode_1024512_motivation.txt"
    ...
    with open(output_filename, "a") as f:
        for batch in batch_sizes:
            ...
            base_command = (
                f"python qserve_benchmark.py "
                f"--model \"{model_path}\" "
                f"--benchmarking "
                f"--precision {precision} "
                f"--group-size {group_size} "
            )
            nsys_output_name = model_path.replace("/", "_").lstrip("_") + f"_bs{batch}——llama2-70b_rebuttal_QServe.nsys-rep"
            command = (
                f"/opt/nvidia/nsight-systems/2024.5.1/bin/nsys profile "
                f"-o {nsys_output_name} "
                f"{base_command}"
            )
